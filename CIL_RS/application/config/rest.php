...
/**********Pushing the auth keys to an array *************/
$authArray = array();
...
if(!is_null($json_str))
{
    ...
    if(!is_null($json) && isset($json->cil_service_users))
    {
        foreach($json->cil_service_users as $user)
        {
            ...
            {
                $authArray[$user->username] = $user->key;
            }
        }
    }
}
/**********End pushing the auth keys to an array *************/
...
|
*/
$config['rest_valid_logins'] = $authArray;
//$config['rest_valid_logins'] = ['cil' => '????'];
